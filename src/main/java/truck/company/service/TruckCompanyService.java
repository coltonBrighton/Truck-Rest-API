...
//  find or create company if there is none
  public Company findOrCreateCompany(Long companyId) { 
...
//  find Company by id if company does not exist throw No such element exception
  public Company findCompanyById(Long companyId) {
    return truckCompanyDao.findById(companyId)
        .orElseThrow(() -> new NoSuchElementException("Company with ID=" + companyId + " does not exist."));
  }
...
//  save company data
  ...
  private void setCompanyFields(Company company, TruckCompanyData truckCompanyData) {
...
//  @formatter:off
    return truckCompanyDao.findAll()
        .stream()
        .map(TruckCompanyData::new)
        .toList();
//   @formatter:on
  }
  ...
  public TruckCompanyData retrieveCompanyById(long companyId) {
    ...
    truckCompanyData.setTrucks(trucks);
    ...
    truckCompanyData.setDrivers(drivers);
  ...
  @Transactional(readOnly = false)
  public TruckCompanyTruck saveTruck(Long companyId, TruckCompanyTruck truckCompanyTruck) {
  ...
  private Truck findTruckById(Long companyId, Long truckId) {
    Truck truck = truckTruckDao.findById(truckId)
        .orElseThrow(() -> new NoSuchElementException("Truck with ID " + truckId + " Does not exist"));
    if (truck.getCompany().getCompanyId().equals(companyId)) {
      return truck;
    } else {
      throw new IllegalArgumentException("Truck does not belong to company with ID " + companyId + ".");
    }
  }

  private void setTruckFields(Truck truck, TruckCompanyTruck truckCompanyTruck) {
  ...
  public List<TruckCompanyTruck> retrieveAllTrucks() {
    return truckTruckDao.findAll().stream().map(TruckCompanyTruck::new).toList();
  }
...
//  Delete the truck from its assigned drivers
    truck.getDrivers().forEach((driver) -> driver.getTrucks().remove(truck));
...
//  save changes
  ...
  public List<TruckCompanyDrivers> retrieveAllDrivers() {
    return truckDriversDao.findAll().stream().map(TruckCompanyDrivers::new).toList();
  }

  @Transactional(readOnly = true)
  public TruckCompanyDrivers retrieveDriversById(Long companyId, Long driverId) {
    Drivers driver = findDriverById(companyId, driverId);
    TruckCompanyDrivers truckCompanyDrivers = new TruckCompanyDrivers(driver);
//  Get a all trucks that are assigned to each driver 
  ...
  private Drivers findDriverById(Long companyId, Long driverId) {
    Drivers driver = truckDriversDao.findById(driverId)
        .orElseThrow(() -> new NoSuchElementException("Truck with ID " + driverId + " Does not exist"));
    if (driver.getCompany().getCompanyId().equals(companyId)) {
      return driver;
    } else {
      throw new IllegalArgumentException("driver does not belong to company with ID " + companyId + ".");
    }
  }

  public TruckCompanyDrivers saveDrivers(Long companyId, TruckCompanyDrivers truckCompanyDrivers) {
    Company company = findOrCreateCompany(companyId);
  ...
  private void setDriverFields(Drivers driver, TruckCompanyDrivers truckCompanyDrivers) {
...
//  delete the driver from their assigned trucks
    driver.getTrucks().forEach((truck) -> truck.getDrivers().remove(driver));
