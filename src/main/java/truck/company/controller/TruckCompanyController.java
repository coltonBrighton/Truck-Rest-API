...
// createCompany by passing in the mapping of the DTO TruckCompanyData
  public TruckCompanyData createCompany(@RequestBody TruckCompanyData truckCompanyData) {
...
//  return the mapped result of the company being deleted successfully and companyId
    return Map.of("message", "Company deleted successfully", "companyId", companyId.toString());
  }
...
//	Get(Read) all trucks
// map trucks to /truck_driving/company/{companyId}/trucks
  @GetMapping("/company/{companyId}/trucks")
...
//  map getting truck by id to /truck_driving/company/{companyId}/trucks/{truckId}
  @GetMapping("/company/{companyId}/trucks/{truckId}")
  public TruckCompanyTruck getTruckById(Long companyId, Long truckId) {
...
//  return http code 201
  ...
  public TruckCompanyTruck truckCompanyTruck(@PathVariable Long companyId,
    ...
    log.info("Creating truck {} at company with ID={}", truckCompanyTruck, companyId);
...
//  map updating a company to /truck_drivers/company/{companyId}/truck/{truckId}
  @PutMapping("/company/{companyId}/truck/{truckId}")
  public TruckCompanyTruck updateTruck(@PathVariable Long companyId, @PathVariable Long truckId,
      @RequestBody TruckCompanyTruck truckCompanyTruck) {
...
//  map deleting a company to /truck_drivers/company/{companyId}/truck/{truckId}
  @DeleteMapping("/company/{companyId}/truck/{truckId}")
  public Map<String, String> deleteTruckById(@PathVariable Long truckId, @PathVariable Long companyId) {
    log.info("Deleting truck with ID {}", truckId);
    truckCompanyService.deleteTruckById(companyId, truckId);
    return Map.of("message", "Truck deleted successfully", "truckId", truckId.toString());
  }
...
//  map get driver by id to /truck_drivers/company/{companyId/driver/{driverId}
  @GetMapping("/company/{companyId}/driver/{driverId}")
  public TruckCompanyDrivers getDriversById(Long companyId, Long driverId) {
...
//  map create a driver to /truck_drivers/company/{companyId}/driver
  ...
  public TruckCompanyDrivers truckCompanyDriver(@PathVariable Long companyId,
    ...
    log.info("Creating driver {} at company with ID={}", truckCompanyDrivers, companyId);
...
//  map deleteDriverById to /truck_drivers/company/{companyId}/driver/{driverId}
  ...
  public Map<String, String> deleteDriverById(@PathVariable Long driverId, 
    ...
    return Map.of("message", "Driver deleted successfully", "driverId", driverId.toString());
  }
...
//  map assigning trucks to drivers to /truck_drivers/truck/{truckId}/{driverId}
    @PutMapping("/truck/{truckId}/{driverId}")
    public Map<String, String> joinTruckAndDriver(@PathVariable Long driverId, @PathVariable Long truckId) {
     log.info("Assigning truck driver with ID {} and truck with ID {}", driverId, truckId);
     truckCompanyService.joinTrucksAndDrivers(driverId, truckId);
     return Map.of("message", "Truck with ID=" + truckId + " has been assigned to Driver with ID=" +  driverId);
    }
    
//  Delete Truck and Driver from join table
//  map unassigning trucks from drivers to /truck_drivers/truck/{truckId}/driver/{driverId} 
    @DeleteMapping("/truck/{truckId}/driver/{driverId}")
    public Map<String, String> deleteTruckFromDriver(@PathVariable Long truckId, @PathVariable Long driverId) {
      log.info("Removing truck with ID {} from driver with ID {}", truckId, driverId);
      truckCompanyService.removeTruckFromDriver(truckId, driverId);
      return Map.of("message", "Truck with ID=" + truckId + " has been unassigned from driver with ID=" + driverId);
    }
